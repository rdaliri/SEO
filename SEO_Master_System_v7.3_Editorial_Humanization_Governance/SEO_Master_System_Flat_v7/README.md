...
7. For publishable content, load `MODULE_08_IMAGE_GENERATION.md`.
8. Run `MODULE_07_QA_AND_RECOVERY.md` as the specialist QA step.
...
2. Create the image prompt package through `MODULE_08_IMAGE_GENERATION.md`.
3. Run `MODULE_07_QA_AND_RECOVERY.md` against the combined deliverable.
...
Load `MODULE_09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` for direct editorial humanization audits, related-batch similarity review, and optional governance of supplied detector results. Use Mode A for direct single-asset auditing, Mode B for related-batch similarity review, and add Mode C only when external detector evidence is supplied. Detector scores never establish authorship or publication eligibility. See `TEAM_MANUAL_MODULE_09.md` for team procedures and copy-paste execution prompts.

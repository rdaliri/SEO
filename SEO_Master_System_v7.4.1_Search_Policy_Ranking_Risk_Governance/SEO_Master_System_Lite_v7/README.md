...
10. Run `modules/07_QA_VERDICT.md` as the specialist QA step.
...
|---|---|
| Humanize or audit one existing article | Controller → canonical context → `modules/02_CONTENT_PRODUCTION.md` when revision is required → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode A → `modules/07_QA_VERDICT.md` → controller verdict |
| Create and audit one new article | Controller → canonical context → `modules/01_INTENT_ARCHITECTURE.md` when intent architecture is needed → `modules/02_CONTENT_PRODUCTION.md` → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode A → `modules/07_QA_VERDICT.md` → controller verdict |
| Create or review a related content batch | Controller → canonical context → `modules/01_INTENT_ARCHITECTURE.md` → `modules/02_CONTENT_PRODUCTION.md` → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode B → `modules/04_LOCAL_AND_SCALED_CONTENT.md` when scaled/local risk applies → `modules/06_AI_SEARCH_VISIBILITY.md` when AEO/GEO applies → `modules/07_QA_VERDICT.md` → controller verdict |
...
| Audit local or scaled landing pages | Controller → canonical context → `modules/01_INTENT_ARCHITECTURE.md` → `modules/02_CONTENT_PRODUCTION.md` when copy is involved → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode B → `modules/04_LOCAL_AND_SCALED_CONTENT.md` → `modules/07_QA_VERDICT.md` → controller verdict |

...
# SEO Master System v7 — Full / Audit Modular File-Aware Edition
...
## Sole Controller
`SEO_MASTER_CONTROLLER_v7.md` is the only authoritative controller in this package. No module, reference file, README, playbook, legacy controller, or orchestration document may override it.

## Required Files
1. `SEO_MASTER_CONTROLLER_v7.md`
2. `CANONICAL_PROJECT_CONTEXT_v7.md`
...
| Final specialist QA, repair, or compact verdict | `modules/07_QA_AND_RECOVERY.md` |
| Post-content image prompt package | `modules/08_IMAGE_GENERATION.md` |
| Editorial humanization, batch similarity, detector-result governance | `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` |
| Search-policy compliance, ranking-risk prevention, diagnosis, recovery | `modules/10_SEARCH_POLICY_AND_RANKING_RISK_GOVERNANCE.md` |
...
2. Load `SEO_MASTER_CONTROLLER_v7.md`.
3. Load `CANONICAL_PROJECT_CONTEXT_v7.md`.
...
9. For publishable content, load `modules/08_IMAGE_GENERATION.md`.
10. Run `modules/07_QA_AND_RECOVERY.md` as the specialist QA step.
11. Let the controller perform evaluation and issue the final verdict.

## Execution and QA Sequence
Controller initialization → canonical project context → required task modules → primary result → Module 09 when triggered → Module 10 when triggered → Module 08 when relevant → Module 07 QA and recovery → controller verdict.
...
## Operating Behavior
Full uses detailed decision, assumption, verification, defect, and iteration registers for complex, client-facing, regulated, scaled, or high-risk work.
...
Load `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` when the project requires any of the following:
...
|---|---|
| Humanize or audit one existing article | Controller → canonical context → `modules/02_CONTENT_PRODUCTION.md` when revision is required → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode A → `modules/07_QA_AND_RECOVERY.md` → controller verdict |
| Create and audit one new article | Controller → canonical context → `modules/01_INTENT_ARCHITECTURE.md` when intent architecture is needed → `modules/02_CONTENT_PRODUCTION.md` → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode A → `modules/07_QA_AND_RECOVERY.md` → controller verdict |
| Create or review a related content batch | Controller → canonical context → `modules/01_INTENT_ARCHITECTURE.md` → `modules/02_CONTENT_PRODUCTION.md` → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode B → `modules/04_LOCAL_AND_SCALED_CONTENT.md` when scaled/local risk applies → `modules/06_AI_SEARCH_VISIBILITY.md` when AEO/GEO applies → `modules/07_QA_AND_RECOVERY.md` → controller verdict |
...
| Audit local or scaled landing pages | Controller → canonical context → `modules/01_INTENT_ARCHITECTURE.md` → `modules/02_CONTENT_PRODUCTION.md` when copy is involved → `modules/09_BATCH_HUMANIZATION_AND_DETECTOR_GOVERNANCE.md` Mode B → `modules/04_LOCAL_AND_SCALED_CONTENT.md` → `modules/07_QA_AND_RECOVERY.md` → controller verdict |

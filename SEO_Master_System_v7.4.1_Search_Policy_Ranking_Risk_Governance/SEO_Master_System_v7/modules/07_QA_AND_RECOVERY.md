...
## v7.4 Mandatory Search Policy Compliance Gate

Before `PASS`, run Module 10 whenever the deliverable is intended for publication, indexation, migration, scaled deployment, structured-data eligibility, link acquisition, or recovery.

Required final checks:

1. Every applicable policy category is `PASS`, `FAIL`, `NOT APPLICABLE`, or `NOT TESTED`.
2. No applicable material category remains `NOT TESTED` unless the controller explicitly blocks publication or issues a condition with owner and retest.
3. Scaled content, doorway, scraping, site reputation, thin affiliate, keyword stuffing, link spam, cloaking, hidden content, redirect, fraud, UGC, hacked-content, and structured-data risks are addressed where applicable.
4. People-first purpose, original value, trust, Who/How/Why, and freshness integrity are reviewed.
5. Technical eligibility claims are directly verified or clearly pending.
6. Ranking declines are not called penalties without direct evidence.
7. No ranking, recovery, rich-result, Discover, or AI-citation guarantee is made.

Include the Module 10 compliance matrix and unresolved policy blocks in the final QA record.
